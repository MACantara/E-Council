...
def test_login_rate_limit(rate_limit_client):
    ...
    for _ in range(5):
        response = rate_limit_client.post(
            "/auth/login", data={"users-username-email": "nonexistent", "users-password": "Password123!"}
        )
...
def test_login_lockout_after_five_failed_attempts(rate_limit_client, sample_user):
    ...
    response = rate_limit_client.post(
        "/auth/login", data={"users-username-email": sample_user.users_username, "users-password": "wrong-password"}
    )
