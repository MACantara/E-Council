...
class TestHelpers:
    ...
    def test_allowed_image_file_valid(self):
        ...
        assert allowed_image_file("test.png")
        assert allowed_image_file("test.jpg")
        assert allowed_image_file("test.jpeg")
        assert allowed_image_file("test.gif")

    def test_allowed_image_file_invalid(self):
        """Test image file validation with invalid extensions."""
        from utils.helpers import allowed_image_file

        assert not allowed_image_file("test.pdf")
        assert not allowed_image_file("test.txt")
        assert not allowed_image_file("test")
...
class TestFilters:
    ...
    def test_has_events_matching(self):
        ...
        events = [MockEvent("First Semester", "2023-2024"), MockEvent("Second Semester", "2023-2024")]
