"""Service layer for the meetings module."""
...
def generate_mom_pdf(minutes_of_the_meeting_id):
    ...
    elements.append(
        Paragraph(
            ...
            + (
                f" - {meeting_data.minutes_of_the_meeting_adjourned.strftime('%I:%M %p')}"
    ...
    # Presiding Officer
    elements.append(Paragraph(f"Presiding Officer: {meeting[1]} {meeting[2]}", section_style))
    ...
    if meeting_data.minutes_of_the_meeting_adjourned:
        elements.append(
            Paragraph(
                f"Meeting Adjourned: {meeting_data.minutes_of_the_meeting_adjourned.strftime('%I:%M %p')}",
                section_style,
            )
        )
    elements.append(Spacer(1, 12))
    ...
    if photos:
        ...
        for photo in photos:
            try:
                ...
                current_app.logger.error("Failed to load photo for PDF generation: %s", e, exc_info=True)
                # If image fails to load, fall back to URL
                elements.append(Paragraph(f"• {photo['url']}", section_style))
        elements.append(Spacer(1, 12))
...
def add_minutes_of_the_meeting():
    if request.method == "POST":
        ...
        if meeting_recording:
            ...
            try:
                ...
                prompt = """Please analyze this meeting transcript and provide a response with only:
                1. Summary:
                ...
                current_app.logger.error("Error processing recording with AI: %s", e, exc_info=True)
    ...
    return render_template(
        "minutes-of-meeting/add-minutes-of-the-meeting.html",
...
def update_minutes_of_the_meeting(meeting_id):
    ...
    return render_template(
        "minutes-of-meeting/update-minutes-of-the-meeting.html",
        meeting=meeting,
        academic_years=academic_years,
        photo_documentations=photo_documentations,
        users=users,
        signatories=signatories,
