...
# Initialize AI model and safety settings
...
def add_concept_paper():
    if request.method == "POST":
        ...
        concept_paper_expected_number_of_participants = request.form.get(
        ...
        concept_paper_event_start_date_and_time = datetime.strptime(
            concept_paper_event_start_date_and_time, "%Y-%m-%dT%H:%M"
        )
        concept_paper_event_end_date_and_time = datetime.strptime(
            concept_paper_event_end_date_and_time, "%Y-%m-%dT%H:%M"
        )
        ...
        parent_guardian_consent_department_office_unit = request.form.get(
            "parent-guardian-consent-department-office-unit"
        )
        parent_guardian_consent_dean_immediate_supervisor = request.form.get(
            "parent-guardian-consent-dean-immediate-supervisor"
        )
    ...
    return render_template(
        "concept-papers/add-concept-paper.html", academic_years=academic_years, users=users, signatories=signatories
    )
...
def update_concept_paper(paper_id):
    ...
    learning_journal = LearningJournalForms.query.filter_by(
        learning_journal_forms_concept_paper_forms_id=paper_id
    ).first()
    parent_guardian_consent_form = ParentGuardianConsentForms.query.filter_by(
    ...
    if request.method == "POST":
        ...
        concept_paper.concept_paper_forms_expected_number_of_participants = (
            concept_paper_expected_number_of_participants
        )
        ...
        if parent_guardian_consent_form:
            parent_guardian_consent_form.parent_guardian_consent_forms_name_of_student = (
                parent_guardian_consent_name_of_student
            )
            parent_guardian_consent_form.parent_guardian_consent_forms_course_year_level = (
                parent_guardian_consent_course_year_level
            )
            parent_guardian_consent_form.parent_guardian_consent_forms_id_number = parent_guardian_consent_id_number
            parent_guardian_consent_form.parent_guardian_consent_forms_department_office_unit = (
                parent_guardian_consent_department_office_unit
            )
            parent_guardian_consent_form.parent_guardian_consent_forms_dean_immediate_supervisor = (
                parent_guardian_consent_dean_immediate_supervisor
            )
    ...
    noted_by_college_dean = (
        personnel_in_charge_form.personnel_in_charge_forms_noted_by_college_dean if personnel_in_charge_form else None
    )
    noted_by_sas = personnel_in_charge_form.personnel_in_charge_forms_noted_by_sas if personnel_in_charge_form else None

    return render_template(
        "concept-papers/update-concept-paper.html",
        concept_paper=concept_paper,
        academic_years=academic_years,
        users=users,
        signatories=signatories,
        objectives_of_the_activity=objectives_of_the_activity,
...
def generate_concept_body():
    ...
    try:
        ...
        try:
            ...
            current_app.logger.error("Invalid date format in generate body: %s", e, exc_info=True)
        ...
        current_app.logger.error(f"Error generating concept paper body: {str(e)}")
...
def generate_concept_descriptions():
    ...
    try:
        ...
        current_app.logger.error(f"Error generating descriptions: {str(e)}")
        return make_response(jsonify({"error": str(e)}), 500)
...
def generate_concept_objectives():
    ...
    try:
        ...
        current_app.logger.error(f"Error generating objectives: {str(e)}")
        return make_response(jsonify({"error": str(e)}), 500)
...
def generate_concept_learning_outcomes():
    ...
    try:
        ...
        current_app.logger.error(f"Error generating learning outcomes: {str(e)}")
        return make_response(jsonify({"error": str(e)}), 500)
...
def generate_concept_participants():
    ...
    try:
        ...
        current_app.logger.error(f"Error generating participant number: {str(e)}")
        return make_response(jsonify({"error": str(e)}), 500)
...
def generate_concept_consent():
    ...
    try:
        ...
        try:
            ...
            current_app.logger.error("Invalid date format in generate consent: %s", e, exc_info=True)
            return make_response(jsonify({"error": "Invalid date format"}), 400)
        ...
        current_app.logger.error(f"Error generating consent form: {str(e)}")
        return make_response(jsonify({"error": str(e)}), 500)
...
def generate_concept_paper_pdf(concept_paper_id):
    ...
    doc = SimpleDocTemplate(
        buffer,
        pagesize=folio_size,  # Use custom FOLIO size
        rightMargin=72,
        leftMargin=72,
        topMargin=100,
        bottomMargin=72,
    ...
    available_width = folio_size[0] - (72 * 2)  # Folio width (8.5") minus left and right margins
    ...
    normal_style = ParagraphStyle("NormalStyle", parent=styles["Normal"], fontSize=12, leading=14, spaceAfter=12)

    header_style = ParagraphStyle(
        "HeaderStyle", parent=styles["Normal"], fontSize=12, leading=14, spaceAfter=6, fontName="Helvetica-Bold"
    )

    # Define wrapped text style
    wrapped_style = ParagraphStyle(
        "WrappedStyle", parent=styles["Normal"], fontSize=12, leading=14, spaceBefore=0, spaceAfter=0
    )
    ...
    routing_data = [
        [
            "FOR:",
            Paragraph(
                f"<b>{concept_paper.approved_by_signatory.signatory_first_name} {concept_paper.approved_by_signatory.signatory_last_name}</b>",
                wrapped_style,
            ),
        ],
        [
            "",
            Paragraph(
                f"{concept_paper.approved_by_signatory.signatory_position}{', ' + concept_paper.approved_by_signatory.signatory_department if concept_paper.approved_by_signatory.signatory_department else ''}",
                wrapped_style,
            ),
        ],
        ["", ""],
        [
            "THRU:",
            Paragraph(
                f"<b>{concept_paper.recommending_approval_by_signatory.signatory_first_name} {concept_paper.recommending_approval_by_signatory.signatory_last_name}</b>",
                wrapped_style,
            ),
        ],
        [
            "",
            Paragraph(
                f"{concept_paper.recommending_approval_by_signatory.signatory_position}{', ' + concept_paper.recommending_approval_by_signatory.signatory_department if concept_paper.recommending_approval_by_signatory.signatory_department else ''}",
                wrapped_style,
            ),
        ],
        ["", ""],
        [
            "FROM:",
            Paragraph(
                f"<b>{concept_paper.endorsed_by_signatory.signatory_first_name} {concept_paper.endorsed_by_signatory.signatory_last_name}</b>",
                wrapped_style,
            ),
        ],
        [
            "",
            Paragraph(
                f"{concept_paper.endorsed_by_signatory.signatory_position}{', ' + concept_paper.endorsed_by_signatory.signatory_department if concept_paper.endorsed_by_signatory.signatory_department else ''}",
                wrapped_style,
            ),
        ],
        ["", ""],
        ...
        [
            "DATE:",
            Paragraph(
                f"{concept_paper.concept_paper_forms_date.strftime('%B %d, %Y')}"
                if concept_paper.concept_paper_forms_date
                else "",
                wrapped_style,
            ),
        ],
    ...
    # Create bullet style
    bullet_style = ParagraphStyle("BulletStyle", parent=normal_style, leftIndent=30, firstLineIndent=-12, spaceAfter=6)
    ...
    details_data = [
        [
            ...
            ":",
            Paragraph(
                f"{concept_paper.concept_paper_forms_event_start_date_and_time.strftime('%A, %B %d, %Y')} (Tentative), {concept_paper.concept_paper_forms_event_start_date_and_time.strftime('%I:%M %p')} - {concept_paper.concept_paper_forms_event_end_date_and_time.strftime('%I:%M %p')}",
                wrapped_style,
            ),
        ],
        [
    ...
    signature_style = ParagraphStyle(
        "SignatureStyle", parent=styles["Normal"], fontSize=12, alignment=1, spaceAfter=0, width=available_width
    )
    ...
    cell_style = ParagraphStyle("CellStyle", parent=normal_style, leading=12, spaceBefore=6, spaceAfter=6)
    ...
    structure_data = [
        ...
        [
            "",
            Paragraph(
                "<br/>".join([f"{i + 1}. {str(obj or '')}" for i, obj in enumerate(objectives)])
                or "No objectives listed",
                cell_style,
            ),
            Paragraph(
                "<br/>".join([f"{i + 1}. {str(outcome or '')}" for i, outcome in enumerate(learning_outcomes)])
                or "No learning outcomes listed",
                cell_style,
            ),
        ],
        [
    ...
    signatories_data = [
        ...
        [
            Paragraph(
                f"<b>{concept_paper.prepared_by_user.users_first_name} {concept_paper.prepared_by_user.users_last_name}</b>",
                centered_signature_style,
            ),
            Paragraph(
                f"<b>{concept_paper.signed_and_reviewed_by_user.users_first_name} {concept_paper.signed_and_reviewed_by_user.users_last_name}</b>",
                centered_signature_style,
            ),
        ],
        [
            Paragraph(
                f"{concept_paper.prepared_by_user.users_student_organization_position}, {get_org_acronym(StudentOrganizations.query.get(concept_paper.prepared_by_user.users_student_organization).student_organizations_name) if concept_paper.prepared_by_user.users_student_organization else ''}",
                centered_signature_style,
            ),
            Paragraph(
                f"{concept_paper.signed_and_reviewed_by_user.users_student_organization_position}, {get_org_acronym(StudentOrganizations.query.get(concept_paper.signed_and_reviewed_by_user.users_student_organization).student_organizations_name) if concept_paper.signed_and_reviewed_by_user.users_student_organization else ''}",
                centered_signature_style,
            ),
        ],
        ["", ""],
    ...
    activity_table = Table(
        activity_details, colWidths=[available_width * 0.4, available_width * 0.3, available_width * 0.3]
    )
    ...
    excuse_letter = ExcuseLetterForms.query.filter_by(
        excuse_letter_forms_concept_paper_forms_id=concept_paper.concept_paper_forms_id
    ).first()
    ...
    if not excuse_letter:
        ...
        personnel_name = (
            excuse_letter.personnel_in_charge_signatory.signatory_first_name
            + " "
            + excuse_letter.personnel_in_charge_signatory.signatory_last_name
            if excuse_letter.personnel_in_charge_signatory
            else ""
        )
        dean_name = (
            excuse_letter.dean_signatory.signatory_first_name + " " + excuse_letter.dean_signatory.signatory_last_name
            if excuse_letter.dean_signatory
            else ""
        )
    ...
    dept_info = [
        [
            Paragraph(
                f"<b>Department/Office Unit:</b><br/>{excuse_letter.excuse_letter_forms_department_office_unit}",
                normal_style,
            ),
    ...
    letter_table = Table(
        letter_data,
        colWidths=[available_width * 0.20, available_width * 0.40, available_width * 0.20, available_width * 0.20],
    )
    ...
    event = Events.query.filter_by(events_concept_paper_forms_id=concept_paper.concept_paper_forms_id).first()
    ...
    events_date = (
        event.events_start_date_and_time.strftime("%A, %B %d, %Y") if event.events_start_date_and_time else "N/A"
    )
    events_time = (
    ...
    section_header_style = ParagraphStyle(
        "SectionHeader", parent=styles["Heading2"], fontSize=12, spaceBefore=15, fontName="Helvetica"
    )
    ...
    data = [
        ...
        [
            Paragraph(
                f"<b>Dean/Faculty in-charge/Officer in- charge:</b><br/>{personnel_in_charge or 'N/A'}", cell_style
            ),
    ...
    for i in range(max_length):
        ...
        objectives_data.append([Paragraph(objective, cell_style), Paragraph(outcome, cell_style)])
    ...
    story.append(
        Paragraph(
            "III. <b>EVALUATION</b>: (Make sure to have three answers for the strengths and weakness.) Weaknesses must have corresponding recommendations.",
    ...
    if not strengths and not weaknesses and not recommendations:
        ...
        for _ in range(3):
            evaluation_data.append([Paragraph("", cell_style), Paragraph("", cell_style), Paragraph("", cell_style)])
    else:
        ...
        for i in range(max_length):
            strength = f"{i + 1}. {strengths[i]}" if i < len(strengths) else ""
            weakness = f"{i + 1}. {weaknesses[i]}" if i < len(weaknesses) else ""
            recommendation = f"{i + 1}. {recommendations[i]}" if i < len(recommendations) else ""
    ...
    signature_data = [
        [Paragraph("Prepared by:", signatory_style), Paragraph("Noted by:", signatory_style)],
        [
            Paragraph(
                "<b>"
                + (
                    ...
                    + " "
                    + (
                        documentation_data.prepared_by_user.users_middle_name + " "
                        if documentation_data.prepared_by_user.users_middle_name
                        else ""
                    )
                    + documentation_data.prepared_by_user.users_last_name
                    + (
                        ", " + documentation_data.prepared_by_user.users_suffix
                        if documentation_data.prepared_by_user.users_suffix
                        else ""
                    )
                )
                + "</b>"
            ...
            Paragraph(
                "<b>"
                + documentation_data.noted_by_signatory.signatory_first_name
                + " "
                + documentation_data.noted_by_signatory.signatory_last_name
                + "</b>"
    ...
    learning_journal_data = [
        # First row - 3 columns
        [
            Paragraph("<b>Name of Student:</b><br/><br/>", cell_style),
            Paragraph("<b>Course/Year level:</b><br/><br/>", cell_style),
            Paragraph("<b>ID Number:</b><br/><br/>", cell_style),
        ],
        # Second row - 2 columns (2nd and 3rd merged)
        [
            Paragraph("<b>College/Department:</b><br/>College of Computer Studies", cell_style),
            Paragraph(
                f"<b>Faculty in-charge:</b><br/>{personnel_in_charge if learning_journal_form else ''}", cell_style
            ),
        ],
        # Third row - 3 columns
        [
            ...
            Paragraph(
                f"<b>Day:</b><br/>{learning_journal_form.learning_journal_forms_date.strftime('%A, %B %d, %Y') if learning_journal_form and learning_journal_form.learning_journal_forms_date else ''}",
                cell_style,
            ),
    ...
    story.append(
        Paragraph(
            "III. Over-all Reflection (The reflection must be written in a narrative form not less than 5 sentences.)",
            section_header_style,
        )
    )
    story.append(Spacer(1, 10))
    ...
    signature_data = [
        ...
        [
            Image(prepared_by_user.signature.get("url"), width=100, height=40)
            if prepared_by_user and prepared_by_user.signature and prepared_by_user.signature.get("url")
            else Paragraph("___________________", signature_style),
            Image(seen_and_read_by_user.signature.get("url"), width=100, height=40)
            if seen_and_read_by_user and seen_and_read_by_user.signature and seen_and_read_by_user.signature.get("url")
            else Paragraph("__________________________", signature_style),
        ],
        # Third row - Titles
        [
        ...
        [
            Paragraph(
                "<b>"
                + (checked_by_signatory.signatory_first_name.upper() if checked_by_signatory else "")
                + " "
                + (checked_by_signatory.signatory_last_name.upper() if checked_by_signatory else "")
                + "</b><br/>"
                + (checked_by_signatory.signatory_position if checked_by_signatory else "")
                + ", "
                + (checked_by_signatory.signatory_department if checked_by_signatory else ""),
                position_style,
            ),
            "",
        ],
    ...
    picf_details = [
        ...
        [
            ...
            Paragraph(f"<b>Venue:</b><br/>{concept_paper.concept_paper_forms_location}", normal_style),
            Paragraph(
                f"<b>College/Department:</b><br/>{concept_paper.department.departments_name if concept_paper.department else ''}",
                normal_style,
            ),
        ],
    ]
    ...
    list_style = ParagraphStyle("ListStyle", parent=normal_style, leftIndent=20, spaceBefore=6, spaceAfter=6)

    # Query personnel in charge form
    pic_form = PersonnelInChargeForms.query.filter_by(
        personnel_in_charge_forms_concept_paper_forms_id=concept_paper.concept_paper_forms_id
    ).first()

    if pic_form:
        personnel_name = (
            f"{pic_form.personnel_in_charge_signatory.signatory_first_name} {pic_form.personnel_in_charge_signatory.signatory_last_name}"
            if pic_form.personnel_in_charge_signatory
            else ""
        )
        dean_name = (
            f"{pic_form.noted_by_dean_signatory.signatory_first_name} {pic_form.noted_by_dean_signatory.signatory_last_name}"
            if pic_form.noted_by_dean_signatory
            else ""
        )
        sas_name = (
            f"{pic_form.noted_by_sas_signatory.signatory_first_name} {pic_form.noted_by_sas_signatory.signatory_last_name}"
            if pic_form.noted_by_sas_signatory
            else ""
        )
    else:
        personnel_name = ""
    ...
    signature_table = Table(
        signature_data, colWidths=[available_width * 0.2, available_width * 0.5, available_width * 0.3]
    )
    signature_table.setStyle(
        TableStyle(
            [
    ...
    pg_form = ParentGuardianConsentForms.query.filter_by(
        parent_guardian_consent_forms_concept_paper_forms_id=concept_paper.concept_paper_forms_id
    ).first()
    ...
    if pg_form:
        ...
        dean_name = (
            f"{pg_form.dean_immediate_supervisor_signatory.signatory_first_name} {pg_form.dean_immediate_supervisor_signatory.signatory_last_name}"
            if pg_form.dean_immediate_supervisor_signatory
            else ""
        )
    else:
        student_name = ""
    ...
    dept_info = [
        ...
        [
            Paragraph(
                f"<b>Faculty In-Charge:</b><br/>{personnel_name if 'personnel_name' in locals() else ''}", normal_style
            ),
    ...
    if pg_form and pg_form.dean_immediate_supervisor_signatory:
        ...
        story.append(
            Paragraph(
                pg_form.parent_guardian_consent_forms_department_office_unit or "OIC Dean, College of Computer Studies",
                normal_style,
            )
        )
        ...
        # Letter to Parents
    story.append(PageBreak())
    story.append(Paragraph(f"<b>{department}</b>", ParagraphStyle("CenteredStyle", parent=normal_style, alignment=1)))
    story.append(Spacer(1, 20))
    ...
    if pg_form and pg_form.parent_guardian_consent_forms_content:
        ...
        letter_text = """<para firstLineIndent='32'></para>"""
    ...
    if pg_form and pg_form.prepared_by_user:
        ...
        org_name = (
            StudentOrganizations.query.get(
                pg_form.prepared_by_user.users_student_organization
            ).student_organizations_name
            if pg_form.prepared_by_user.users_student_organization
            else ""
        )
        org_acronym = get_org_acronym(org_name)
        story.append(Paragraph(f"<b>{prepared_by_name.upper()}</b>", normal_style))
        story.append(
            Paragraph(f"{pg_form.prepared_by_user.users_student_organization_position}, {org_acronym}", normal_style)
        )
    story.append(Spacer(1, 20))
    ...
    if pg_form and pg_form.noted_by_signatory:
        noted_by_name = (
            f"{pg_form.noted_by_signatory.signatory_first_name} {pg_form.noted_by_signatory.signatory_last_name}"
        )
        story.append(Paragraph(f"<b>{noted_by_name.upper()}</b>", normal_style))
        story.append(
            Paragraph(
                f"{pg_form.noted_by_signatory.signatory_position}, {pg_form.parent_guardian_consent_forms_department_office_unit}",
                normal_style,
            )
        )
    ...
    story.append(Spacer(1, 12))
    story.append(Paragraph("Reply Slip", ParagraphStyle("CenteredStyle", parent=normal_style, alignment=1)))
    story.append(Spacer(1, 12))

    # Create checkbox style with ZapfDingbats
    checkbox_style = ParagraphStyle("CheckboxStyle", parent=normal_style, leftIndent=20)
    ...
    buffer.seek(0)
    return send_file(buffer, download_name=f"Concept_Paper_{concept_paper_id}.pdf", mimetype="application/pdf")
