...
class MinutesOfTheMeetingForm(FlaskForm):
    ...
    def __init__(self, academic_years=None, student_organizations=None, signatories=None, users=None, **kwargs):
        ...
        if student_organizations is not None:
            ...
            self.minutes_of_the_meeting_presiding_officer.choices = [
            ...
            self.minutes_of_the_meeting_noted_by.choices = [
                ("", "Select a Signatory"),
                ("add-new-noted-by", "Add new signatory"),
            ] + [
                (
        ...
        if users is not None:
            self.minutes_of_the_meeting_approved_by.choices = [("", "Select a User")] + [
                (
                    str(user.users_id),
                    f"{user.users_first_name} {user.users_last_name} - {user.users_student_organization_position}",
                )
                for user in users
                if user.users_role == "Student Council Officer"
            ]
