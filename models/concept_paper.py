...
class ConceptPaperForms(db.Model):
    ...
    recommending_approval_by_signatory = db.relationship(
        "Signatories", foreign_keys=[concept_paper_forms_recommending_approval_by]
    )
...
class ExcuseLetterForms(db.Model):
    ...
    personnel_in_charge_signatory = db.relationship(
        "Signatories", foreign_keys=[excuse_letter_forms_personnel_in_charge_forms_id]
    )
