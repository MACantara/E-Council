...
class BoardResolutions(db.Model):
    ...
    board_resolutions_date = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    board_resolutions_events_id = db.Column(db.Integer, db.ForeignKey("events.events_id"), nullable=True, index=True)
    board_resolutions_departments_id = db.Column(db.Integer, db.ForeignKey("departments.departments_id"), nullable=True, index=True)
    board_resolutions_title = db.Column(db.String(255), nullable=True)
    board_resolutions_academic_year = db.Column(db.String(50), nullable=True, index=True)
    board_resolutions_semester = db.Column(db.String(50), nullable=True, index=True)
    board_resolutions_status = db.Column(db.String(50), nullable=True, index=True)
    ...
    board_resolutions_prepared_by = db.Column(db.Integer, db.ForeignKey("users.users_id"), nullable=True, index=True)
    board_resolutions_approved_by = db.Column(db.Integer, db.ForeignKey("signatories.signatory_id"), nullable=True, index=True)
